...
public:
    ...
    void Start(unsigned thrCount){
        ...
        for (auto it = receivers.begin(); it != receivers.end(); ++it, i++)
            receiver_threads.emplace_back([=]{ ReceiverThreadFunction(i + 1, *it); });
        ...
        for (auto it = transmitters.begin(); it != transmitters.end(); ++it, i++)
            transmitter_threads.emplace_back([=]{ TransmitterThreadFunction(i + 1, *it); });
    }
...
int main(void){
    ...
    std::cout << std::fixed << std::setprecision(2) << "CPU time used: " << 1000.0 * (double)(c_end - c_start) / CLOCKS_PER_SEC << " ms\n"
