...
public:
    ...
    void AddFragment(uint64_t x){
        uint32_t fragment_id = x >> SHIFT_MSG_ID;
        {
            ...
            to_process.emplace_back(fragments[fragment_id]);     /* TODO: copying the vector */
        }
        rw_cv.notify_one();
    }
...
private:
    ...
    deque<pair<uint32_t, const CBigInt &>> to_send;                  /* queue of pairs to be sent by transmitters */
    ...
    void WorkerThreadFunction(unsigned thID){
        while(!to_process.empty() || !all_fragments_sent){
            ...
            printf("W%d started with %zu fragments, id %d.\n", thID, fragments_to_process.size(), group_id);
            ...
            if (number_of_payloads == 0){
                ...
                printf("W%d: incomplete result for group %d.\n", thID, group_id);
                continue;
            }

            {
                unique_lock<mutex> lk (ic_mutex);
                auto it = incomplete_ids.find(group_id);
                if (it != incomplete_ids.cend())
                    incomplete_ids.erase(it);
            }

            printf(" @ W%d is computing result for group %d.\n", thID, group_id);
            CBigInt max_result;
            for (auto &pair : bitfields) {
                CBigInt tmp = CountExpressions(pair.first, pair.second);
                if (tmp.CompareTo(max_result) > 0)
                    max_result = tmp;
                delete [] pair.first;
            }
    ...
    void TransmitterThreadFunction(unsigned thID, const ATransmitter & transmitter){
        while(!to_send.empty() || !all_work_completed){
            ...
            printf(" <-- T%d sending result for group %d.\n", thID, id_to_send);
            transmitter->Send(id_to_send, count_to_send);
        }
        ...
        while(ic_it != incomplete_ids.cend()){
            ...
            printf(" <-- T%d: incomplete result for group %d.\n", thID, incomplete_id);
